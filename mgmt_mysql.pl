...
post '/mysql/databases' => sub {
    ...
    unless ($DRH->func('createdb', $DB->quote_identifier($name), $config->{db}->{host}, $config->{db}->{user}, $config->{db}->{password}, 'admin')) {
        $c->rendered(409);
        return;
    }

    my $sth = $DB->prepare_cached("GRANT ALL PRIVILEGES ON " . $DB->quote_identifier($name) . ".* TO ?\@'%'");
